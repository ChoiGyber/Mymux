...
unsafe fn public_window_callback_inner<T: 'static>(
  ...
  let keyboard_callback = || {
    ...
    // `process_message` calls `PeekMessageW`, which delivers pending *sent* messages —
    // and `WM_SETFOCUS` / `WM_KILLFOCUS` are sent messages that `is_msg_keyboard_related`
    // accepts. So this window procedure re-enters while `process_message` is running. The
    // builder is therefore borrowed out of the map instead of held under the global lock:
    // a reentrant call gets `None` and returns no events, rather than parking this thread
    // on a lock it already owns. See issue #36.
    let events = crate::platform_impl::platform::keyboard::with_key_event_builder(
      WindowId(window.0 as _),
      |key_event_builder| key_event_builder.process_message(window, msg, wparam, lparam, &mut result),
    )
    .unwrap_or_default();
    for event in events {
      subclass_input.send_event(Event::WindowEvent {
