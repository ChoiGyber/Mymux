...
/// Puts a borrowed [`KeyEventBuilder`] back into [`KEY_EVENT_BUILDERS`], even if the
/// borrower unwinds. Losing the builder would silently disable keyboard input for that
/// window for the rest of its life.
struct BorrowedBuilder {
  window_id: WindowId,
  builder: Option<KeyEventBuilder>,
}

impl Drop for BorrowedBuilder {
  fn drop(&mut self) {
    if let Some(builder) = self.builder.take() {
      KEY_EVENT_BUILDERS.lock().insert(self.window_id, builder);
    }
  }
}

/// Runs `f` with the window's [`KeyEventBuilder`] **without holding the global
/// [`KEY_EVENT_BUILDERS`] lock while `f` runs**.
///
/// `KeyEventBuilder::process_message` calls `PeekMessageW`, and `PeekMessageW` delivers
/// pending *sent* messages to the window procedure before it returns. `WM_SETFOCUS` and
/// `WM_KILLFOCUS` are sent messages and are keyboard-related (see
/// [`is_msg_keyboard_related`]), so the window procedure re-enters this path while `f` is
/// still running. `parking_lot` mutexes are not reentrant, so holding the lock across `f`
/// parks the UI thread on a lock it already owns — permanently, with no CPU use and no
/// error. Taking the builder out of the map keeps the locked region to the lookup itself.
///
/// A re-entrant call finds no builder and gets `None` instead of blocking. The nested
/// message then produces no key event, exactly like the pre-existing "window is not in the
/// map" branch. That is correct here: `PeekMessageW` only dispatches *sent* messages, and
/// real keystrokes are *posted*, so a nested invocation is never a genuine keypress.
  ...
  pub(crate) fn process_message(
    ...
    match msg_kind {
      ...
      win32wm::WM_KEYUP | win32wm::WM_SYSKEYUP => {
        ...
        // Same reentrancy hazard as the key-down branch: do not hold LAYOUT_CACHE across
        // PeekMessageW. See issue #36.
        let event_info = {
          let mut layouts = LAYOUT_CACHE.lock();
...
mod reentrancy_tests {
  use super::{with_key_event_builder, KeyEventBuilder, KEY_EVENT_BUILDERS};
  use crate::platform_impl::platform::WindowId;
  use std::panic::{catch_unwind, AssertUnwindSafe};

  // Mymux issue #36. `KeyEventBuilder::process_message` calls `PeekMessageW`, which
  // delivers pending *sent* messages — and `WM_SETFOCUS` / `WM_KILLFOCUS` are sent
  // messages that `is_msg_keyboard_related` accepts. The window procedure therefore
  // re-enters this lookup while the outer borrow is still live. If the global map lock
  // were held across the borrow, the reentrant lookup would park the UI thread on a lock
  // that same thread already owns: no CPU use, no error, no recovery.
  #[test]
  fn reentrant_lookup_returns_instead_of_blocking() {
    let id = WindowId(0x3600_0001);
    KEY_EVENT_BUILDERS.lock().insert(id, KeyEventBuilder::default());

    let outer = with_key_event_builder(id, |_outer_builder| {
      // This is the reentrant window-procedure call. It must come back.
      with_key_event_builder(id, |_nested_builder| "nested borrowed the builder")
    });

    assert_eq!(
      outer,
      Some(None),
      "the outer borrow must succeed and the reentrant one must return None"
    );
    assert!(
      KEY_EVENT_BUILDERS.lock().contains_key(&id),
      "the builder must be back in the map once the outer borrow ends"
    );
    KEY_EVENT_BUILDERS.lock().remove(&id);
  }

  // Losing the builder would silently disable keyboard input for that window forever, so
  // an unwinding borrower must still put it back.
  #[test]
  fn builder_is_restored_when_the_borrower_panics() {
    let id = WindowId(0x3600_0002);
    KEY_EVENT_BUILDERS.lock().insert(id, KeyEventBuilder::default());

    let result = catch_unwind(AssertUnwindSafe(|| {
      with_key_event_builder(id, |_builder| panic!("borrower blew up"))
    }));

    assert!(result.is_err(), "the panic must propagate to the caller");
    assert!(
      KEY_EVENT_BUILDERS.lock().contains_key(&id),
      "the builder must survive a panicking borrower"
    );
    KEY_EVENT_BUILDERS.lock().remove(&id);
  }

  #[test]
  fn unknown_window_yields_none() {
    assert!(with_key_event_builder(WindowId(0x3600_0003), |_| ()).is_none());
  }
}
