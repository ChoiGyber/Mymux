...
# 3부 · SFTP 가 따라오지 않던 원인 (구현 중 추가된 범위)

사용자가 "SFTP 가 제대로 안 된다, `clp` 로 확인해 달라" 고 해서 같이 고쳤다.

## 3.1 측정한 것

| 대상 | 결과 |
|---|---|
| OpenSSH `sftp clp` | 0.74초, `/root` 목록 정상 |
| 같은 russh 경로(`examples/sftp_probe.rs`), 올바른 키 | 0.86초, 7단계 전부 통과 |
| `~/.mycli/known_hosts` 의 clp 호스트키 | 서버 실제 키와 동일 |
| 실행 중 빌드 | v0.1.57 — 이슈 #11 수정 포함 |

SFTP 스택 자체는 멀쩡했다. 문제는 **`ssh` 를 친 걸 감지하지 못하는 것**과
**엉뚱한 키를 고르는 것** 두 가지였다.

## 3.2 타이핑한 ssh 를 놓치던 이유

`app.js` 에는 입력 추적기가 둘이다.

| 추적기 | 방식 | ↑ 히스토리 |
|---|---|---|
| `getInputRegion` | OSC 133 마크 기준으로 화면을 직접 읽음 | 인식함 |
| `acInput` | 눌린 키를 문자열에 누적 | 방향키에서 `return` — 인식 못 함 |

`adoptTypedSsh`/`syncExplorerOnCd` 가 `acInput` 쪽에 물려 있어서, ↑ 로 불러
실행한 `ssh clp` 는 `typed === ""` 가 되어 `/^ssh\s/` 에 걸리지 않았다.
아무 일도 안 일어나고 오류도 없다.

`submittedCommandLine(ti)` 를 만들어 화면을 읽고, 제출 라인을 그쪽에서도 쓴다.

**우선순위는 키스트로크가 먼저다.** 화면 우선으로 짰다가 `exit` 감지가 깨졌다 —
ssh 세션 안에서는 OSC 133 마크가 **스크롤돼 사라진 로컬 셸의 것**이라, 화면을
읽으면 원격 프롬프트에 명령이 붙은 문자열이 나와 `/^(exit|logout)$/` 에 영영
안 걸린다. 키스트로크가 있으면 그게 이 패인이 실제로 보낸 것이므로 항상 옳고,
화면은 키스트로크가 구조적으로 볼 수 없는 경우(↑ 복원, 셸 자동완성)만 메운다.

## 3.3 키 자동탐지

`explorer.rs` 가 `id_ed25519`, `id_ecdsa`, `id_rsa` **세 이름만** 시도했다.
호스트별 키(`id_ed25519_clp`, `id_rsa_safetyabc`)는 후보에 들지도 못하고,
이름이 같은 `id_ed25519` 는 서버가 거부한다(프로브로 확인).

`auto_key_candidates()` 로 바꿨다. 표준 3종을 먼저 시도하고, 그다음 `~/.ssh` 에서
**짝이 되는 `.pub` 가 있는 파일**을 이름순으로 시도한다. 그 짝이 `id_ed25519_clp`
와 `config`·`known_hosts` 를 이름 추측 없이 갈라준다. sshd `MaxAuthTries`(기본 6)
를 넘기면 연결이 끊기므로 4개로 자른다.

## 3.4 조용한 실패

대상 해석에 성공했는데 SFTP 연결이 실패하면 패인·대상당 한 번 토스트로 이유를
알린다. 해석 자체가 안 되는 명령은 지금처럼 아무 말도 하지 않는다 — 사용자가
요청한 동작이 아니기 때문이다.
