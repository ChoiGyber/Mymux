...
## 이미 구현된 조각 (2026-08-11)

이 검토의 "1단계 안전" 접근을 그대로 따르는 첫 기능이 들어갔습니다:
🔔 알림 설정 모달의 **Claude statusline 연동** 버튼(`src/statusline.rs`).
없을 때만 추가하고, 남의 statusline은 절대 교체하지 않으며, 수정 전
`settings.json.mymux-bak` 백업을 남기고, 끄기로 되돌릴 수 있습니다. 위에 적은
안전 규칙(옵트인·덮어쓰기 금지·되돌리기)의 동작 예시로 삼을 수 있습니다.
