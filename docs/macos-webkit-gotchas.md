...
## 7. 자동 가드 (CI) — 이 문서의 불변조건을 기계로 지킨다

이 문서 §1~§4의 "현재 코드" 항목이 소스에서 사라지면(리팩터링 중 실수로 지우는 게
전형적 재발 경로) **`scripts/check-macos-gotchas.mjs`** 가 잡아낸다:

```sh
node scripts/check-macos-gotchas.mjs   # 로컬에서 아무 때나 (의존성 없음)
```

CI에도 걸려 있어 따로 안 돌려도 된다:

| 워크플로 | 시점 |
|----------|------|
| `ci.yml` (`macos-gotchas` job) | main push / PR 마다 |
| `release.yml` | 릴리즈 태그 빌드 직전 |
| `macos-build.yml` | 맥 dmg 빌드 직전 (해당 태그 체크아웃 기준) |
...
새 함정을 발견하면 이 문서에 절을 추가하고 **같은 커밋에서 체크 항목도 추가**할 것.
